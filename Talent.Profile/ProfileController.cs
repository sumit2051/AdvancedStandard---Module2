    ...
    public class ProfileController : Controller
    {
        ...
        public ActionResult GetLanguages()
        {
            try
            {
                ...
                var result =  _userLanguageRepository.GetQueryable().Where(x => x.UserId == userId).AsEnumerable().Select(x => ViewModelFromLanguage(x)).ToList();
        ...
        //            var existingUser = _userRepository.GetByIdAsync(talentId).Result;
        ...
        //            existingUser.Languages.Add(newLanguage);
        ...
        public async Task<IActionResult> GetTalentSnapshots(FeedIncrementModel feed)
        {
            try
            {
                ...
                var sum = _userRepository.Collection.Count();
                ...
                //        );
                //}
                return Json(new { success = true, data = result, total = sum });
            }
            catch (Exception e)
            {
                return Json(new { success = false, e.Message });
